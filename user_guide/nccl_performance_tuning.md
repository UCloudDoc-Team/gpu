...
!> UCloud已对GPU云主机底层虚拟化进行升级，可通过lspci命令确认是否存在QEMU PCIe Expander bridge确认。部分存量运行的云主机，在控制台上关机/开机一次之后, 将自动为您完成升级。若不进行升级，测试nccl时需要指定NCCL_TOPO_FILE为/var/run/nvidia-topologyd/virtualTopology.xml (docker场景下也请透传此文件)。
